...
async def on_ready():
    ...
    await bot.change_presence(
        status=discord.Status.online, activity=discord.Game(f"GnK봇 {version}")
    )
...
class Map:
    ...
    def get_data(self, mode, amount):
        if mode == "normal":
            ...
            maplist = self.veryhard
        elif mode == "all":
            maplist = self.mapall
        elif mode == "item":
            maplist = self.mapitem
        else:
            maplist = "normal, hard, veryhard, all, item"

        if amount > len(maplist):
            return f"{mode}는 {len(maplist)}개입니다."

        result = random.sample(maplist, amount)
...
async def 맵추첨(ctx, mode, amount):
    maplist = maps.getmap(mode, int(amount))
    await ctx.send(f"{maplist}")
...
async def 문의(ctx):
    ...
    await guild.create_text_channel(
        "문의 " + str(count), overwrites=overwrites, category=cate
    )
