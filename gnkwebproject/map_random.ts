...
function map_random(mode,amount){
    ...
    for(var i=0; i<mapresult.length; i++)
        document.getElementById("result").innerHTML+=`<div id="${i+1}"><button type="button" onclick="picked(${i+1}); document.getElementById('${i+1}').style.visibility='hidden';">${i+1}</button>${mapresult[i]}</div>`;
    
    for(var i=1; i<=mapresult.length; i++)
        number.push(i);
}